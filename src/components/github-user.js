...
const GithubUser = (props) => {
  ...
  props.repositoriesContributedTo.edges.forEach(({ node }) => {
    const primaryLanguage = node && node.primaryLanguage && node.primaryLanguage.name
  ...
  const l2 = []
  let r
  for (r in languages) { l2.push([r, languages[r]]) }

  // sort by number of appearances
  const l3 = l2.sort((a, b) => {
    if (a[1] > b[1]) { return 1 }
    if (a[1] < b[1]) { return -1 }
    return 0
  })
  ...
  return <div style={{ padding: '1em', margin: '1em', border: 'thin solid blue' }}>
    ...
    <dl>
      ...
      <dt>Étoiles</dt>
      <dd>{props.starredRepositories && props.starredRepositories.edges.length} dépots</dd>

      <dt>Contribué à</dt>
      <dd>{props.repositoriesContributedTo && props.repositoriesContributedTo.edges.length} dépots</dd>
